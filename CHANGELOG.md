...
- **314 automated tests** — 247 unit, 67 integration against a real VS Code instance.
- Character mode uses a hand-rolled extended grapheme cluster walk rather than
  `Intl.Segmenter`. The Segmenter is pathologically slow on some Node/V8 builds — CI
  measured 85 seconds to segment 500 KB on Node 20, against 45 ms for this walk — which
  would have frozen the extension host on a large character-mode insertion. Emoji ZWJ
  sequences, skin-tone modifiers, flags, keycaps, variation selectors, combining marks
  and CRLF pairs are all still kept intact.
